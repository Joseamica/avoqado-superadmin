...
# Opcional: VITE_GOOGLE_CLIENT_ID (mismo client ID del web-dashboard) habilita
# el botón "Continuar con Google" del login. Sin él, sólo email + contraseña.
...
# Ver PRODUCT.md para la dirección estética y los anti-patrones prohibidos.
```
...
**Entrar con Google** (opcional — requiere `VITE_GOOGLE_CLIENT_ID`): el botón oficial de Google Identity Services devuelve un ID token en el navegador y lo mandamos a `POST /api/v1/dashboard/auth/google/one-tap`, que emite las mismas cookies. Sin client ID configurado, la sección entera del LoginPage no se renderiza.
...
| `/settlement-calendar`                                      | Depósitos: calendario mensual de cuánto liquida cada día y a qué negocio (todos los venues)                                        | `settlement-calendar` |
...
| `/merchants` · `/merchants/:id`                             | Registro de merchant accounts + detalle (editor completo según proveedor, economía + asistente de pricing, liquidación, readiness) | `merchants`           |
...
**Después de instalar**: re-estiliza al design system (`PRODUCT.md`) — los defaults de shadcn son reconociblemente AI-genéricos.
