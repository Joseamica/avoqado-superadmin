...
- **H4 (audit FK linkage):** newly-created `TransactionCost` rows (CREATE_COST + newProviderRates) set `providerCostStructureId`/`venuePricingStructureId` to the **pre-correction** structure ids while storing the **new** rates. Capture and use the newly-created structure id. _Audit-linkage only; amounts are correct._
