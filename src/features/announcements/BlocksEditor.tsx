      ...
      {bloques.map((b, i) => (
        <div key={i} className="rounded-[8px] border border-[var(--line-strong)] p-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[12px] uppercase tracking-wide text-[var(--ink-faint)]">{b.type}</span>
            <div className="flex gap-0.5">
              <IconButton size="sm" onClick={() => mover(i, -1)} aria-label="Subir bloque" disabled={i === 0}>
                <ArrowUp className="h-3.5 w-3.5" aria-hidden="true" />
              </IconButton>
              <IconButton size="sm" onClick={() => mover(i, 1)} aria-label="Bajar bloque" disabled={i === bloques.length - 1}>
            ...
            <div className="space-y-1.5">
              {b.items.map((it, k) => (
                <input
                  ...
                  onChange={e => reemplazar(i, { ...b, items: b.items.map((x, m) => (m === k ? e.target.value : x)) })}
                />
              ))}
              <button type="button" className="inline-flex h-8 items-center px-1 text-[12px] text-[var(--ink-muted)] transition-colors hover:text-[var(--ink)]" onClick={() => reemplazar(i, { ...b, items: [...b.items, ''] })}>
                + otro punto
              </button>
            </div>
          )}
            ...
            <div className="space-y-1.5">
              {b.rows.map((r, k) => (
                <div key={k} className="flex gap-1.5">
                  <input className={input} value={r.label} placeholder="Pantallas" onChange={e => reemplazar(i, { ...b, rows: b.rows.map((x, m) => (m === k ? { ...x, label: e.target.value } : x)) })} />
                  <input className={input} value={r.value} placeholder="2 táctiles" onChange={e => reemplazar(i, { ...b, rows: b.rows.map((x, m) => (m === k ? { ...x, value: e.target.value } : x)) })} />
                </div>
              ))}
