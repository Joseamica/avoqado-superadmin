...
export function PricingWizardDrawer({ open, onOpenChange, ...wizard }: Props) {
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        {/* El asistente vive DENTRO del contenido, que Radix monta al abrir y desmonta al
            cerrar: su estado se siembra en cada apertura con el `cost` y los `venues` de ESE
            momento, y cerrar (venga de donde venga) descarta el paso y lo tecleado. Con el
            estado en el componente de arriba —que la página monta desde el primer render,
            antes de que cargue el costo— la PRIMERA apertura arrancaba con ceros e IVA
            marcado, y sin venue destino; `reset()` sólo corría al cerrar, así que la segunda
            apertura sí salía bien (hermano del defecto de «Editar economía», 2026-09-14). */}
        <PricingWizard onOpenChange={onOpenChange} {...wizard} />
      </DrawerContent>
    </Drawer>
  )
}

function PricingWizard({ onOpenChange, cost, venues, onPrefill }: Omit<Props, 'open'>) {
  ...
  function handlePrefill() {
    ...
    // Cerrar desmonta este componente: no hay estado que reiniciar.
    onOpenChange(false)
  }
          ...
          <section className="flex flex-col gap-4">
            ...
            {venues.length === 0 ? (
              <p className="text-[13px] text-[var(--ink-faint)]">
                Este merchant no está asignado a ningún venue todavía. Asígnalo desde el detalle de
                un venue para poder aplicarle este pricing.
              </p>
            ) : (
              <div>
