...
- **Página Ganancias (`/earnings`).** Cuánto gana Avoqado, combinando los dos buckets de ingreso: **terminales** (`TransactionCost.grossProfit` — el spread venue−proveedor) **+ cobros en línea** (`CheckoutSession.applicationFeeCents` — la comisión de plataforma). Filtrable por rango de fechas (`DateRangePicker` con presets; default mes en curso), con KPIs de cabecera (ganancia bruta + split terminal/online, volumen, margen, transacciones), tendencia (`recharts`, día/semana/mes) y desglose en tabs (`DataTable`): por negocio, merchant account, proveedor, tipo de tarjeta y canal online. La ganancia se etiqueta **"bruta"** (no descuenta el corte del agregador, que no se persiste por transacción). Entrada de sidebar "Ganancias" en el grupo Operación. Nuevo helper reusable `src/shared/lib/money.ts` (`formatMoney`/`formatCompactMoney`, MXN).
  - **Backend (`avoqado-server`, aditivo):** nuevos endpoints `GET /api/v1/superadmin/earnings/summary` y `/time-series` con su propio `earnings.service.ts`, que **reusa tal cual** `paymentAnalyticsService` para el profit terminal y **crea** las queries de online + los desgloses completos (por venue/merchant). Cumple la nueva "Namespace rule" de `CLAUDE.md`: el front sólo consume `/superadmin/*`, nunca `/dashboard/superadmin/*`.
