...
- **Asignar serials (magic link): los labels de "Unidad N — nombre" y "Serial físico" ahora sí están asociados a sus inputs.** Los `<Field>` del loop de unidades en `AssignSerialsTpvOrderPage` no pasaban `id` ni `name`, así que el `htmlFor` del label quedaba `undefined`: clic en el label no enfocaba el input y los lectores de pantalla no anunciaban la etiqueta. Ahora cada unidad recibe un `id` único (`${item.id}-${idx}-name` / `${item.id}-${idx}-serial`) y los tests consultan los inputs vía `getByLabelText`, que verifica la asociación.
- **Wizard de alta de merchant: arreglado el crash `(e.terminals ?? []).map is not a function` al pulsar "Crear merchant".** El endpoint `blumon/full-setup` no devuelve un `MerchantAccount`: responde un **resumen** de orquestación donde `terminals` es un objeto de conteos (`{autoAttached, batchAttached, total}`), pero el front lo pasaba por `mapMerchant`, que hacía `.map` sobre ese objeto. **El merchant SÍ se creaba** server-side (HTTP 201) — el error era sólo el cliente reventando al leer la respuesta, por lo que no navegaba al detalle. `fullSetupBlumon`/`fullSetupAngelPay` ahora parsean su forma real (`{ merchantAccountId, … }`); el wizard AngelPay tenía el mismo bug latente (navegaba a `/merchants/undefined`). Sin cambios de backend.
...
- **Tests de integración para las páginas de magic-link de TPV orders.** `ApproveTpvOrderPage`, `RejectTpvOrderPage` y `AssignSerialsTpvOrderPage` (rutas públicas `/admin/tpv-orders/:id/approve|reject|assign-serials`, fuera del `ProtectedRoute`) ahora tienen cobertura Vitest + RTL + MSW: aprobación exitosa (200 con `orderNumber`), token inválido/expirado (401 → estado de error con el mensaje del servidor), el happy path de rechazo (validación del token vía `approve/check` + envío del motivo) y el flujo de asignación de serials (validación vía `assign-serials/check` que renderiza el form con los items del pedido, CTA deshabilitado hasta llenar todos los serials, POST con el payload `{ items: [{ orderItemId, units }] }` y estado de éxito). Reemplaza al spec de Playwright que vivía en `avoqado-web-dashboard` (`e2e/tests/tpv/buy-terminal-spei.spec.ts`) y quedó obsoleto cuando estas páginas se movieron a este repo.
- **Wizard de alta Blumon: elegir terminal existente, atar terminales extra y configurar el reparto de ganancias.** Las dos cards que antes decían "Opcional" pero no hacían nada ahora son interactivas:
  - **Paso "Terminal Blumon"** gana un toggle **Por serial (autofetch)** | **Elegir existente** — en el segundo modo, un `Combobox` lista las terminales registradas del venue y al elegir una pre-llena serial/marca/modelo (el backend igual hace el autofetch de credenciales por ese serial).
  - **Card "Terminales TPV"** abre un drawer de **multi-select** para atar terminales extra del venue a la cuenta (filtradas a la marca del merchant y excluyendo la del serial, que se auto-ata sola) → alimenta `additionalTerminalIds` del `full-setup`.
  - **Card "Reparto de ganancias"** abre un drawer (mismos campos que el editor del detalle, extraídos a un `RevenueShareFields` reusable) para configurar el split directo/agregador en el alta. Se guarda **tras** crear el merchant con el endpoint existente `POST /superadmin/merchant-revenue-shares` (no se tocó `full-setup`); si esa segunda llamada falla, el merchant queda con el default 100% Avoqado y un toast lleva al detalle. Todo frontend; el endpoint `/superadmin/terminals?venueId=` (double-mounted) lista las terminales del venue sin importar el feature `terminals`.
- **Carry-over de acceso de staff al migrar una TPV (`StaffAccessStep` + `GrantAccessDrawer`).** Cuando una terminal se mueve de un venue a otro, las personas que la usaban pierden el login (el PIN es por-venue). Nuevo **paso "staff"** dentro del wizard de migración (`pick → staff → preflight → progress`): tras elegir el destino, el operador da acceso a las personas que usaban la terminal — un `Combobox` busca a la persona (las del venue origen aparecen primero, marcadas "usaba esta terminal"), y cada una se vuelve una fila con un `Combobox` de **rol** (pre-seleccionado con el rol que tenía en el origen, labels en español vía `ROLE_LABEL_ES`; nunca ofrece OWNER/SUPERADMIN) y un input de **PIN** (4-6 dígitos, pre-llenado con un PIN sugerido libre), más una frase plana ("Le vas a dar acceso a X como Mesero con PIN 1234") para que un operador no-técnico entienda qué pasará. El paso se puede **Omitir**; al dar acceso, el blocker `NO_STAFF_PIN` del preflight ya pasa porque alguien con PIN quedó en el destino. El mismo paso está disponible **standalone** desde el drawer de acciones de la terminal → sección "Acceso" → "Dar acceso a una persona" (sin venue origen: el picker muestra a toda la organización, nada pre-seleccionado). Los mensajes de error del backend (PIN duplicado / PIN en uso) se muestran verbatim vía `toast.error`. Endpoints consumidos (ya existentes en el backend, namespace `/superadmin/*`): `GET /superadmin/venues/:id/staff-access/candidates?sourceVenueId=`, `POST /superadmin/venues/:id/staff-access`. Hooks nuevos: `useVenueAccessCandidates` (query) + `useGrantVenueAccess` (mutation, invalida candidatos y el listado de terminals).

- **Asistente "Migrar TPV a otro venue" (`TerminalMigrationDrawer`).** Desde el drawer de acciones de una terminal, nueva sección "Migración" → "Migrar a otro venue" abre un wizard de 3 pasos: **(1) elegir destino** — `Combobox` de venues (excluye el venue actual de la terminal) + picker opcional de merchant accounts a asignar (se oculta si el destino no tiene cuentas; la terminal hereda el primario del venue); **(2) preflight** — verifica el destino y renderiza bloqueadores (banners danger, bloquean continuar) y advertencias (banners warn), con una compuerta destructiva de **confirmación tipiada** (el operador escribe el serial para autorizar el factory-reset + re-vinculación); **(3) progreso** — hace polling del estado mostrando un checklist (comando entregado / rebote post-restablecimiento / online bajo el nuevo venue) y una nota de timeout ~10 min cuando la terminal no reaparece; mientras no se confirma ofrece **"Cancelar migración"** (restaura el venue original, con toast del error del backend si ya es tarde), y al confirmar muestra "Finalizar". El badge **"Migrando"** aparece en la columna de estado de `/terminals` para terminals con migración en vuelo, y abrir su drawer reanuda el asistente directo en el paso de progreso. Endpoints consumidos (ya existentes en el backend): `POST /dashboard/superadmin/terminals/:id/migrate-preflight`, `.../migrate-execute`, `GET .../migrate-status?commandId=`, `POST .../migrate-cancel`; la lista de terminals ahora surface el campo `migration` por terminal. Hooks nuevos: `useMigratePreflight` / `useMigrateExecute` / `useMigrateCancel` (mutations, invalidan el listado) + `useMigrateStatus` (query con `refetchInterval` que se detiene al confirmar).
